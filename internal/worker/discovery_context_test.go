...
func TestProductionDecisionContextKeepsCrossBranchEvidenceDiscoverable(t *testing.T) {
	for _, entry := range []string{"snapshot", "cursor"} {
		for _, mode := range []string{"initial", "incremental", "closure_over_budget"} {
			t.Run(entry+"/"+mode, func(t *testing.T) {
				...
				if mode != "initial" {
					previous = &before
					cursor = &board.DecisionCursor{ProjectID: current.Graph.Project.ID, Generation: current.Graph.Project.Generation, Revision: before.Revision}
					if mode == "incremental" {
						// The needed facts are unchanged history in other branches.
						before.FactRecords[90].Description = "The previous unrelated inventory observation."
						before.Graph.Facts[90].Description = before.FactRecords[90].Description
				...
				var decision *board.DecisionContext
				var err error
				if entry == "snapshot" {
					decision, err = board.BuildDecisionContext(current, previous, events, board.DefaultContextViewBytes)
				} else {

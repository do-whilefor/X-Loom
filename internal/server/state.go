...
func (s *Server) stateAction(t *b.Tx, q *request, r *http.Request) (int, any, error) {
	for key := range q.fields {
		if key != "op" && key != "idempotency_key" && key != "payload" {
	...
	result, err := t.StateAction(r.PathValue("pid"), b.ExecutionFence{Run: r.Header.Get("X-Xloom-Run"), Lease: r.Header.Get("X-Xloom-Lease"), Intent: r.Header.Get("X-Xloom-Intent")}, b.StateAction{Op: op, IdempotencyKey: key, Payload: raw})
